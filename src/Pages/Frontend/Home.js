...
export default function Home() {
    ...
    const [isLiked, setIsLiked] = useState(false); // For Handling Liking system
    ...
    const handleLikes = async (post) => {
        ...
        if (currentUser) {
            ...
            const isUserLiked = updatedPost.likes.some((like) => like.likedBy === currentUser.userId);
            setIsLiked(isUserLiked);
            if (isUserLiked) {
                updatedPost.likes = updatedPost.likes.filter((like) => like.likedBy !== currentUser.userId);
            } else {
                updatedPost.likes.push({ likedBy: currentUser.userId });
            }
            newArr = posts.map((doc) => {
